...
-- Записувати може лише сам учень (свій рядок)
create policy "signal: insert own progress"
  on public.signal_progress for insert
  with check (auth.uid() = user_id);

create policy "signal: update own progress"
  on public.signal_progress for update
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

-- Якщо таблицю вже створено попередньою версією скрипту, замість create table
-- виконайте лише це (замінює старе правило читання):
--
